...
/*
 Java Program to convert decimal number to Roman numerals
 */
public class RomanNumber {
	...
	// To add corresponding base symbols in the array to handle cases that follow
	// subtractive notation. Base symbols are added index 'i'.
		...
		while (number != 0) {
			...
			if (number >= 1000) {
				...
				if (number < 900) {
					...
					number = number % 500;
				} // To handle subtractive notation in case of number having digit as 9 and adding
				...
				if (number < 400) {
					...
					number = number % 100;
				} // To handle subtractive notation in case of number having digit as 4 and adding
					// corresponding base symbol
				else {
					i = sub_digit('C', 'D', i, c);
					number = number % 100;
